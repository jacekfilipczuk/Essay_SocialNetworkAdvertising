...
// Importante: Deallocare la memoria delle varie clique.
...
// Variabili globali
Graph *g;
Clique Best, *C_Best,*C_Best2;
//Lista_Candidati NLC,NIC;
Heap *NLC, *NIC;
int numSteps = 0,numCliques;
...
Clique *multistart;
int index_multistart,num_max_saved_clique=50;
float range;
...
void Dynamic_Local_Search(Graph *g, Clique *C, Clique *C1,int nodo_Start_Clique, bool dalla_terza_fase);

bool NodesReduction(Graph *g,Clique *C_best); //Restituisce true se ha eliminato almeno un nodo inutile dal grafo dinamico

int NeighborhoodWeight(Node *n);
...
void Inizializza_Array_Clique(Clique *array,int array_size,  int n, int ncolors);
...
int main(int argc, char *argv[]) {
    ...
    if (argc == 7) {
        ...
        // printf("NODO INIZIALE CLIQUE %f",g->GetSingleNode(2)->GetPeso());
        ...
        Inizializza_Array_Clique(multistart,num_max_saved_clique,numNodes, numColors);
        ...
        Inizializza_Clique(C_Best2,numNodes,numColors);

        NLC = (Heap*) new Heap(numNodes + 1);
        NIC = (Heap*) new Heap(numNodes + 1);
        NIL = (Coppia*) malloc(sizeof (Coppia) * (numNodes*numNodes));
        ...
        //do{
                    //***** DA MODIFICARE LA FASE 1 ****//
            // Fase 1
        Node *ptr;
        //Calcola il peso del vicinato per ogni nodo
        for(int i=0; i<numNodes;i++){
                ptr=g->GetSingleNode(i);
                ptr->setPeso_Neighbourhood(NeighborhoodWeight(ptr));
        }



        for(int i=0; i<numNodes; i++){// eseguo multistart dell'algoritmo per stabilizzare risultati

            Svuota_Clique(C);
            Svuota_Clique(C1);
            NIC->Clean_Heap();
            NLC->Clean_Heap();
            g->setGrafo_dinamico(g->getGrafo_iniziale());
            Dynamic_Local_Search(g, C, C1,i,trovata_new_max_clique);
        }

        trovata_new_max_clique=false;
        if(C_Best2->size<=C_Best->size){
            Svuota_Clique(C_Best2);
        }

       // Stampa_Clique(C_Best2,0);
            // Fase 2
            //Riduzione_Grafo(g, C_Best, C_Best->colori_nella_clique);

            // TODO: per avere un multistart efficace bisogna usare la tabella hash per controllare che 
            //       tutte le soluzioni siano differenti.
            // Fase 3
            //for(int i=0;i<index_multistart;i++){
                //Stampa_Clique(C_Best,0);
             //   Color_Reduction(g,&multistart[i]);
            //}
            
        //}while(trovata_new_max_clique==true);
        //printf("\n\n");
        //printf("NODO DEL GRAFO ORIGINALE:  %d ",g->GetSingleNode(1)->GetPeso()); I nodi del grafo non hanno un peso in questo punto del codice
        //Stampa_Soluzione(C_Best, print_option);
        printf("Info sul grafo  \t Nodi: %d \t Archi:%d \t Tempo: %.2f ",g->GetNumnodes(),g->GetNumarcs(),timer()-t);
        printf("Clicque massima \t Size: %d \t Peso: %.2f\t Numero Clique massime trovate: %d \t ",C_Best->size,C_Best->peso_clique,numCliques);
        printf("Seconda Clicque massima \t Size: %d \t Peso: %.2f\t",C_Best2->size,C_Best2->peso_clique);
        printf("\n\n");
        
    } else {
        printf("\n\nERROR: Wrong number of parameters!!!\n");
        printf("USAGE: alg input_file range tcs penalty_delay maxsteps print_option\n");
        exit(0);
    }
...
void Dynamic_Local_Search(Graph *g, Clique *C, Clique *C1,int nodo_Start_Clique, bool dalla_terza_fase) {
    ...
    nodoIniziale = Find_nodo_in_G(nodo_Start_Clique, C);
    ...
    while (numSteps < maxSteps) {
        ...
        if (C->peso_clique > C_Best->peso_clique) {
            numCliques=0;
...
//            }
            ...
            index_multistart=0;
            Copia_Clique(C,&multistart[index_multistart]);
            
        }
        else if(C->peso_clique == C_Best->peso_clique){
            numCliques++;

            if( C->size > C_Best->size) Copia_Clique(C, C_Best);
            else if( C->size == C_Best->size && index_multistart < num_max_saved_clique){
                Copia_Clique(C,&multistart[index_multistart]);
                index_multistart++;
            }
        }
        //Controllo se la clique attuale è più piccola di un certo range rispetto all'ottima incombente ma ha una cardinalità maggiore
        if(C->peso_clique>=(C_Best->peso_clique-((C_Best->peso_clique)*range)) && C->size>C_Best->size){
            Copia_Clique(C,C_Best2);
        }
...
//#endif			
        ...
        else if(ultimo_aggiunto!=NULL && penalty_delay > 1 && i_random(1,100)>10){
            Svuota_Clique(C);
...
//Funzione che rimuove tutti i nodi dal grafo dinamico il cui peso insieme a quello dei vicini è più piccolo del peso della clique (è come se eliminasse tutti i nodi che formano delle clique con i vicini e il cui peso totale è minore della clique ottima trovata
bool NodesReduction(Graph *g, Clique *C_best){

    bool flag=false;
    float peso_clique=0,peso_vicinato=0;
    Node *g_dinamico;
    peso_clique = C_best->peso_clique;
    g_dinamico = g->getGrafo_dinamico();
    while(g_dinamico!= NULL){
        peso_vicinato =NeighborhoodWeight(g_dinamico);
        if(peso_vicinato<peso_clique){

            g->Rimuovi_Nodo_Dyn(g_dinamico);
            flag=true;
        }
        g_dinamico = g_dinamico->GetNodo_down_dyn();
    }
    return flag;

}

//Funzione che calcola il peso di un nuodo e di tutti i vicini del nodo
int NeighborhoodWeight(Node *n){

    Arc *archi=NULL;
    int peso_tot=n->GetPeso();
    archi=n->GetCorona_dyn();
    while(archi!= NULL){
        peso_tot+=archi->GetNodeTo()->GetPeso();
...
void Color_Reduction(Graph *g, Clique *Current_C){
    ...
    Copia_Clique(Current_C,C_Start);
        ...
        if(C_Start->array_nodi[da_inserire->GetIndex()].e_nella_clique==false){
            ...
            if(C_Start->array_nodi[da_inserire->GetIndex()].incidenza==C_Start->size){
                ...
                while(ptr != NULL){
                    ...
                    Incrementa_Occorrenze_Colori(da_rimuovere, C_Start);
                    if (colori_rimossi > colori_inseriti) {
                        NIL[index_NIL].da_inserire = da_inserire;
                        NIL[index_NIL].da_rimuovere = da_rimuovere;
                        NIL[index_NIL].key = colori_inseriti-colori_rimossi;
                        index_NIL++;
...
void Copia_Clique(Clique *originale, Clique *copia) {
    ...
    copia->peso_clique = originale->peso_clique;
...
// #endif       
        ...
        ins->e_nella_clique = true;
        C->size++;
        C->peso_clique+= da_inserire->GetPeso();
...
void Rimuovi_da_C(Node *da_rimuovere, Clique *C) {
    ...
    if (rmv->e_nella_clique == true) {
        ...
        rmv->e_nella_clique = false;
        C->size -= 1;
        C->peso_clique-= da_rimuovere->GetPeso();
...
//    Stampa_NIC_NLC(NLC);
    ...
    rmv->era_nella_clique_iniziale = true;
    C->size -= 1;
    C->peso_clique-= rmv->nodo_ptr->GetPeso();
    ...
    ins->e_nella_clique = true;
    C->size++;
    C->peso_clique+= ins->nodo_ptr->GetPeso();
...
//        }
    ...
    while (NIC->getHeapSize() > 0) {
        temp_estratto = (Node*) NIC->Extract_Heaviest_Neighbourhood();
...
//            if(C->size == C_Best->size && C->colori_nella_clique == C_Best->colori_nella_clique){
//               Copia_Clique(C,&multistart[index_multistart]);
//               index_multistart++;
//            }
//                
...
void Stampa_Clique(Clique *C, short flag) {
    ...
    printf("\nSize= %d, Peso=%.2f, CLIQUE: ", C->size, C->peso_clique);
...
void Stampa_Soluzione(Clique *C, short flag) {
    ...
    if( Verifica_Clique(C)==true ){
        ...
        if (flag == 0) {
            ...
            int ln=sprintf(buffer,"\t %d \t %d \t %d \t %d \t %d \t %d \t %.2f \t %d\t%ld\t%d\tOK!\n", g->GetNumnodes(), g->GetNumarcs() / 2, g->GetNumcolors(), budget, C->size, C->colori_nella_clique, t,nodi_cancellati,miglioramenti_cp,maxSteps);
            ...
            //printf("\t#nodes= %d \t #numarcs= %d \t #numcolors= %d \t budget= %d \t |C|= %d\t l(C)=%d\t time= %.2f \t nodi_cancellati= %d\tmiglioramento_3_fase= %ld\t%d\nCLIQUE(nodo,(peso)): ", g->GetNumnodes(), g->GetNumarcs() / 2, g->GetNumcolors(), budget, C->size, C->colori_nella_clique, t,nodi_cancellati,miglioramenti_cp,maxSteps);
            printf("\t#nodes= %d \t #numarcs= %d \t range= %.2f \t |C|= %d\t time= %.2f \t \nCLIQUE(nodo,(peso)): ", g->GetNumnodes(), g->GetNumarcs() / 2, range, C->size, t);

            ptr = C->testa_clique;
            while (ptr != NULL) {
                printf("%d  ", ptr->nodo_ptr->GetId());
                peso_atteso += ptr->nodo_ptr->GetPeso();
                printf("(%.2f),",ptr->nodo_ptr->GetPeso());
                ptr = ptr->clique_next;
            }

             printf("\n Peso totale clique RESTITUITO: %f \n", C->peso_clique);
             printf("\n Numero di Clique di Peso massimo trovate: %d \n", numCliques);
            //fflush(stdout);
            printf("\tOK!\n");
        }
    }
...
bool Verifica_Clique(Clique *C) {
    ...
    if(C->testa_clique == NULL){
       printf("\n\nERRORE [Verifica_Soluzione]: La clique passata in input e' vuota.!!!\n\n");
       exit(0);    
    }
...
void Inizializza_Array_Clique(Clique *array,int array_size,  int n, int ncolors){
    ...
    int i;
    multistart = (Clique*) malloc(sizeof (Clique) * (array_size+1));
    
    for(i=0;i<array_size;i++){
        //array[i].array_nodi = (Nodo_Clique*) malloc(sizeof (Nodo_Clique) * n);
        //array[i].occorrenze_colori = (int*) malloc(sizeof (int) * (ncolors + 1));
        Inizializza_Clique(&multistart[i],n,ncolors);
    }
